...
import { MusterStackNavigator } from './MusterStackNavigator';
...
const TabStackNavigator = () => {
  ...
  return (
    <Tab.Navigator
      ...
      screenOptions={(screenProps) => ({
        ...
        tabBarIcon: ({ color, size, focused }) => {
          ...
          switch (screenProps.route.name) {
            case 'TrackerHome':
              label = t('Tracker');
              icon = <MaterialCommunityIcons name="counter" size={24} color={focused ? theme.warning : theme.text} />;
              break;
            case 'ArmyBuilder':
              label = t('ArmyBuilder');
              icon = <MaterialCommunityIcons name="sword-cross" size={24} color={focused ? theme.warning : theme.text} />;
              break;
            case 'About':
              label = t('About');
              icon = <AntDesign name="infocirlceo" size={24} color={focused ? theme.warning : theme.text} />;
              break;
            default:
      ...
      <Tab.Screen
        name="Muster"
        component={MusterStackNavigator}
        options={{
          title: 'Army Muster',
          tabBarLabel: 'Army',
          headerShadowVisible: false,
